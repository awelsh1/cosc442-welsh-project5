...
public class Utilities{
  ...
  // copy of runFMS without println statements to focus on on output of JUnit methods. 
  public static void runFSMlite(State [] FSM, int stateID, String input, String separator)
  {
	   // input is a sequence of  symbols  from the input alphabet separated by string in separator.
	    // StateId is the ID of the state to which input is to be applied.
	    String outputPattern="";  // Output sequence generated by the FSM. Illegal input generates empty output.
	    ...
	    //Utilities.debugFSMExecution("Output pattern:"+outputPattern);
	    loop:
	    for (int i = 0; i <= outputPattern.length() - 1; i++)
	    {
	    	if (outputPattern.charAt(i) == 'y') 
	    	{
	    		System.out.print("assertTrue(bondRegex(");
	    		break loop;
	    	}	
	    	
	    	if (i == outputPattern.length() - 1)
	    	{
	    		System.out.print("assertFalse(bondRegex(");
	    		break loop;
	    	}
	    }
